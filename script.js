...
(function initTilt() {
  ...
  if (!card || window.matchMedia('(pointer:coarse)').matches) return; // skip on touch
  ...
  document.querySelector('.card-wrapper').addEventListener('mousemove', e => {
    ...
    raf = requestAnimationFrame(() => {
      ...
      card.style.transform = `perspective(900px) rotateY(${dx * MAX_TILT}deg) rotateX(${-dy * MAX_TILT}deg)`;
    });
  });
